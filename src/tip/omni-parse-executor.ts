...
export type OmniParseOp = 'beautify' | 'minify' | 'clean' | 'normalize';

export function createOmniParseTipExecutor(op: OmniParseOp) {
  return async (input: TIPBundle, config: TIPConfig, hooks: TIPHooks): Promise<TIPBundle> => {
    hooks.onProgress(0, `Initializing OmniParse ${op}...`);
