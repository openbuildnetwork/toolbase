...
# ==================================================
# FILE: tools\\redact-secrets\\redact-secret.py
# ==================================================
import re
import math
...
def detect_user_keys(text, keys):
    ...
    for key in keys:
        pattern = re.compile(
            rf'{re.escape(key)}\\s*[:=]\\s*["\\'](?P<v>[^"\\']+)["\\']', re.IGNORECASE
        )
...
def redact_content(content, contentType, masking_style, user_hints, log_options):
    ...
    for d in sorted(merged, key=lambda x: x.start, reverse=True):
        ...
        masked_content = (
            masked_content[: d.start] + replacement + masked_content[d.end :]
        )
    ...
    return {
        ...
        "summary": {"totalMasked": len(merged), "byType": by_type},
