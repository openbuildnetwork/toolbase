...
export function FavoritesDrawer({ isOpen, onClose }: FavoritesDrawerProps) {
    const { favorites, clearFavorites, toggleFavorite } = useToolPreferences();
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between px-6 py-5 border-b border-white/5 dark:border-white/5">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[12px] font-medium px-1.5 py-0.5 rounded-md bg-red-500/10 text-red-500">
                                    {favorites.length}
                                </span>
                            </div>
                            <button
                            ...
                            {favoriteTools.length > 0 ? (
                                    ...
                                    <AnimatePresence mode="popLayout">
                                        {favoriteTools.map((tool) => (
                                            ...
                                            >
                                                ...
                                                {/* Individual Remove Button (Unfavorite) */}
                                                <button
                                                    onClick={(e) => {
                                                        e.preventDefault();
                                                        e.stopPropagation();
                                                    ...
                                                    title={`Remove ${tool.name} from favorites`}
                                                >
                                                    <X size={14} />
                                                </button>
                                            </m.div>
                                ...
                                <div className="h-full flex flex-col items-center justify-center text-center px-8 opacity-40">
                                    ...
                                    <p className="text-[12px]" style={{ color: 'var(--text-secondary)' }}>
                                        Tools you heart will appear here for quick access.
                                    </p>
                                </div>
                            )}
                        </div>
