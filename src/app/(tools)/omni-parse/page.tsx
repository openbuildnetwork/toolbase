	            ...
	            {activeTab === "generate" && (
	              <div className="grid lg:grid-cols-1 gap-6">
	                        ...
	                        {!docGraph.error && (
	                          <div className="flex flex-wrap items-center gap-2">
	                            ...
	                            <Button
	                              variant="outline"
	                              className="h-8 rounded-full px-3 text-xs font-semibold"
	                              onClick={() => setDocGraphExpandedPaths(new Set())}
	                            >
	                              Collapse All
	                            </Button>
	                            <Button
	                ...
	                {isDocGraphModalOpen && (
	                  <div
	                    className={cn(
	                      // Render inside <main> and start below the tool navbar (h-14) so content isn't hidden.
	                      "absolute top-14 bottom-0 right-0 left-0 z-50 pointer-events-none",
	                      isSidebarOpen && "md:left-[280px]"
	                    )}
	                  >
	                    {/* Cover only the main area (sidebar is separate in the layout) */}
	                    <div className="absolute inset-0 bg-black/20 backdrop-blur-sm pointer-events-auto" onClick={() => setDocGraphModalOpen(false)} />
	                    <div className="absolute inset-0 pointer-events-none flex items-start justify-center md:justify-start p-4 md:py-8 md:pr-8 md:pl-4">
	                      <div
	                        className="pointer-events-auto w-full max-w-6xl h-full bg-white/90 backdrop-blur-2xl border border-black/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
                        ...
                        <div className="h-14 border-b border-gray-200/70 bg-white/60 backdrop-blur-md flex items-center justify-between px-5">
                          <div className="text-sm text-gray-500">
                            ...
                            <span className="text-gray-300">/</span>
                            <span className="ml-2">Input JSON</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Button
                              variant="outline"
