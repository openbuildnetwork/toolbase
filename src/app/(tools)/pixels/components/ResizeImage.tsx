...
import { ImagePreview } from "@/app/(tools)/pixels/components/ImagePreview";
...
export function ResizeImage() {
    ...
    const { execute, isProcessing, tool } = useTIPTool('pixels/resize');
    const isReady = true;
    ...
    useEffect(() => {
        if (!seededRef.current && Object.keys(defaultConfig).length > 0) {
            Promise.resolve().then(() => setConfig(defaultConfig));
    ...
    const handleFileSelect = async (files: File[]) => {
        ...
        try {
            ...
            setOriginalInfo(info as ImageInfo);
    ...
    useEffect(() => {
        if (resizeMode === "percentage" && originalInfo?.width && originalInfo?.height) {
            const factor = percentage / 100;
            Promise.resolve().then(() => {
                setWidth(Math.round((originalInfo.width as number) * factor));
                setHeight(Math.round((originalInfo.height as number) * factor));
            });
        }
    ...
    const handleResize = async () => {
        ...
        try {
            const resizeConfig = {
                ...
                height,
                mode: fitMode,
                fillColor: config.fillColor as string,
            };
                            ...
                            <div className="space-y-6">
                                ...
                                {resizeMode === 'percentage' && (
                                    <div className="space-y-4">
                                        ...
                                        <Slider
                                            value={percentage}
                                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPercentage(Number(e.target.value))}
                                            min={1}
                                            max={200}
                                            step={1}
                                ...
                                <div className="pt-4 border-t border-gray-100 space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Slider
                                            ...
                                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateConfig('quality', Number(e.target.value))}
                                            min={10}
                                            max={100}
                                            step={1}
                            ...
                            <div className="flex justify-between items-center opacity-80 text-sm font-medium">
                                ...
                                {processedInfo && (
                                    <span className="font-mono text-cyan-500">
                                        {formatBytes(processedInfo.size_bytes || 0)}
                                    </span>
                                )}
                            </div>
                            <Button
