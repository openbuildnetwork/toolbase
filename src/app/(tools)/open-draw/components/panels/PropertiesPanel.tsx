...
interface PropertiesPanelProps {
    selectedNodes: Node[];
    selectedEdges: Edge[];
    onNodeChange: (changes: FlexibleData) => void;
    onEdgeChange: (changes: FlexibleData) => void;
    isOpen: boolean;
    onClose: () => void;
}
...
export function PropertiesPanel({ selectedNodes, selectedEdges, onNodeChange, onEdgeChange, isOpen, onClose }: PropertiesPanelProps) {
    ...
    const data: FlexibleData = (isNodeSelected ? (firstNode?.data || {}) : (firstEdge?.data || {})) as FlexibleData;
    const edgeStyle: FlexibleData = (firstEdge?.style || {}) as FlexibleData;
    ...
    const updateStyle = (key: string, value: unknown) => onNodeChange({ [key]: value });
    const handleEdgeStyleChange = (key: string, value: unknown) => onEdgeChange({ style: { ...edgeStyle, [key]: value } });
    const handleEdgeDataChange = (key: string, value: unknown) => onEdgeChange({ data: { ...data, [key]: value } });

    const toggleEdgeMarker = (start: boolean) => {
        const markerKey = start ? 'markerStart' : 'markerEnd';
        const current = (firstEdge as Record<string, unknown>)[markerKey];
            ...
            <div className="flex border-b border-gray-200 dark:border-gray-800">
                {[
                    ...
                    <button
                        ...
                        onClick={() => setActiveTab(tab.id as 'style' | 'text' | 'arrange')}
                    >
...
function NodeStyleView({ data, updateStyle, firstNode, onNodeChange }: { data: FlexibleData, updateStyle: (k: string, v: unknown) => void, firstNode: Node, onNodeChange: (c: FlexibleData) => void }) {
    return (
        <>
            ...
            <div className="space-y-3 pb-4 border-b border-gray-100 dark:border-gray-800">
                <PropertyRow label="Fill">
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <ColorPicker value={String(data.backgroundColor || '#ffffff')} onChange={(c) => updateStyle('backgroundColor', c)} />
                    </div>
                </PropertyRow>
                <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 ml-5 cursor-pointer">
                    <input
                        type="checkbox"
                        checked={Boolean(data.gradient) || false}
                ...
                <PropertyRow label="Line">
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            className="rounded border-gray-300 text-blue-600"
                        />
                        <ColorPicker value={String(data.borderColor || '#000000')} onChange={(c) => updateStyle('borderColor', c)} />
                    </div>
                </PropertyRow>
                <div className="flex gap-2 ml-5">
                    <select
                        value={String(data.borderStyle || 'solid')}
            ...
            <div className="space-y-3 pb-4 border-b border-gray-100 dark:border-gray-800">
                <PropertyRow label="Shape">
                    <select
                        value={String(data.shapeType || 'rectangle')}
            ...
            {data.imageUrl && (
                <div className="space-y-3 pb-4 border-b border-gray-100 dark:border-gray-800 bg-emerald-50/30 dark:bg-emerald-900/10 p-2 rounded-lg">
                    ...
                    <PropertyRow label="Crop Mode">
                        <select
                            value={String(data.imageMode || 'cover')}
                    ...
                    <PropertyRow label="Zoom">
                        <input
                            ...
                            value={Number(data.imageZoom || 100)}
            ...
            <div className="space-y-3 pt-2">
                <PropertyRow label="Opacity">
                    <input
                        type="range" min="0" max="100" value={Number(data.opacity ?? 100)}
                    ...
                    />
                    <span className="text-xs text-gray-500 w-8 text-right">{Number(data.opacity ?? 100)}%</span>
                </PropertyRow>
                ...
                <div className="space-y-3">
                    <PropertyRow label="Rounded">
                        <div className="flex items-center gap-2">
                            ...
                            {!!data.borderRadius && (
                                <input
                                    type="number" min="0" max="100" value={Number(data.borderRadius)}
...
function StyleActions({ firstNode, onNodeChange }: { firstNode: Node, onNodeChange: (c: FlexibleData) => void }) {
    return (
...
function EdgeStyleView({ edgeStyle, handleEdgeStyleChange, toggleEdgeMarker, firstEdge, onEdgeChange }: { edgeStyle: FlexibleData, handleEdgeStyleChange: (k: string, v: unknown) => void, toggleEdgeMarker: (s: boolean) => void, firstEdge: Edge, onEdgeChange: (c: FlexibleData) => void }) {
    return (
        <div className="space-y-4">
            <div className="space-y-2">
                <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block">Line Style</label>
                <div className="flex gap-2 items-center mb-2">
                    <ColorPicker value={String(edgeStyle.stroke || '#b1b1b7')} onChange={(c) => handleEdgeStyleChange('stroke', c)} />
                    <input
                        type="number" min="1" max="10" value={Number(edgeStyle.strokeWidth || 2)}
                ...
                <select
                    ...
                    onChange={(e) => {
                        ...
                        const dash = val === 'dashed' ? '5,5' : (val === 'dotted' ? '2,2' : undefined);
...
function NodeTextView({ data, updateStyle }: { data: FlexibleData, updateStyle: (k: string, v: unknown) => void }) {
    return (
        <div className="space-y-4">
            <PropertyRow label="Font Size">
                <input
                    type="number" value={Number(data.fontSize || 14)}
            ...
            <PropertyRow label="Text Color">
                <ColorPicker value={String(data.textColor || '#000000')} onChange={(c) => updateStyle('textColor', c)} />
            </PropertyRow>
        </div>
    );
}

function EdgeTextView({ data, handleEdgeDataChange }: { data: FlexibleData, handleEdgeDataChange: (k: string, v: unknown) => void }) {
    return (
        <div className="space-y-4">
            <div className="space-y-2">
                <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block">Label</label>
                <input
                    type="text" value={String(data.label || '')}
            ...
            <PropertyRow label="Text Color">
                <ColorPicker value={String(data.textColor || '#000000')} onChange={(c) => handleEdgeDataChange('textColor', c)} />
            </PropertyRow>
...
function ArrangeView({ firstNode }: { firstNode: Node }) {
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
                {['x', 'y'].map(axis => (
                    <div key={axis}>
                        ...
                        <input
                            type="number" value={Math.round(firstNode.position[axis as 'x' | 'y'])}
                            readOnly
