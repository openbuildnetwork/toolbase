...
import type { ShapeDefinition } from '@/app/(tools)/open-draw/types/open-draw.types';
                        ...
                        {(!!searchTerm || expandedCategories.includes(category.name)) && (
                            <div className="grid grid-cols-4 gap-2 px-2 py-2">
                                {category.shapes.map((shape) => {
                                    ...
                                    return (
                                        <div
                                            key={isIcon && 'data' in shape && shape.data ? shape.data.iconName : shape.type}
                                            ...
                                            onDragStart={(e) => {
                                                if (isIcon) {
                                                    ...
                                                    if (isIcon && 'data' in shape && shape.data) {
                                                        e.dataTransfer.setData('application/open-draw-data', JSON.stringify({
                                                            iconName: shape.data.iconName,
                                                            iconType: 'iconType' in shape.data ? shape.data.iconType : undefined
                                                        }));
                                                    }
                                                } else {
                                                    onDragStart(e, shape.type);
                                                }
                                            }}
                                            title={shape.label}
                                        >
