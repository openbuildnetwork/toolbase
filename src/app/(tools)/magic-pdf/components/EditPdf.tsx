...
interface WorkerDetectionElement {
    id: string;
    type: ElementType;
    content: string;
    x: number;
...
interface WorkerDetectionResponse {
    elements: WorkerDetectionElement[];
    width: number;
    height: number;
    error?: string;
}
    ...
    useEffect(() => {
        ...
        const detectElements = async () => {
            try {
                ...
                const results = await magicPdfWorker.execute('detect', { file_bytes: new Uint8Array(fileBytes), page_index: currentPage - 1 }) as WorkerDetectionResponse;
                if (results && Array.isArray(results.elements)) {
                    const newElements: EditElement[] = results.elements.map((res) => {
                        return {
                            id: res.id,
                            type: res.type,
                            content: res.content,
