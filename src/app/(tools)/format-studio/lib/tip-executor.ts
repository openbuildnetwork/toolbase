import { TIPBundle, TIPHooks, TIPConfig, TIPPayload, TIPContentType } from '@/tip/protocol';
    ...
    const processedPayloads = await Promise.all(
      input.payloads.map(async (payload: TIPPayload, i: number) => {
        ...
        try {
          ...
          throw new TIPError('EXECUTION_FAILED', `Failed to ${op} ${payload.meta.filename}: ${message}`);
        }
