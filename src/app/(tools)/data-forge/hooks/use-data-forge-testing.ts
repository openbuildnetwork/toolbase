...
import type { GenerationProfile, JsonSchemaNode, SchemaValidationSummary } from "@/lib/data-forge";
...
export function useDataForgeTesting() {
  ...
  const handleExportFixturePack = () => {
    try {
      ...
      downloadText("data-forge-fixture-pack.json", pack, "application/json");
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Error";
