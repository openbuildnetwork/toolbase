...
import type { Base64Response } from '../types/base64';

interface Base64Result extends Base64Response {
    mode?: string;
    urlSafe?: boolean;
    mimeType?: string;
}
...
export default function Base64Interactive({
    ...
    const [lastResult, setLastResult] = useState<Base64Result | null>(null);
    ...
    const handleConfirm = () => {
        ...
        if (typeof lastResult.result === 'string') {
            ...
            const uint8 = lastResult.result instanceof Uint8Array 
                ? lastResult.result 
                : new Uint8Array(lastResult.result as number[]);
        ...
        onConfirm({
            ...
            config: {
                ...
                mimeType: lastResult.mimeType || 'application/octet-stream',
            },
        });
    };
        ...
        <div className="flex flex-col h-full overflow-hidden font-display p-8 space-y-8 bg-(--background)">
            ...
            <div className="flex-1 min-h-0 overflow-hidden bg-(--surface-overlay)/40 backdrop-blur-md border border-(--border-subtle) rounded-3xl p-6 shadow-2xl">
                <Base64Workspace
                    ...
                    onResultUpdate={(res) => setLastResult(res)}
                />
            </div>
