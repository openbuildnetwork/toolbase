...
function buildTree(value: unknown, rootLabel: string, maxDepth: number, maxNodes: number) {
  ...
  const makeNode = (
    ...
    return { id: `n-${counter}`, label, subLabel, path, childCount, children };
  };
  ...
  const visit = (nodeValue: unknown, label: string, path: string, depth: number): GraphNode => {
    ...
    if (depth >= maxDepth) {
      return makeNode(label, `${sub} (max depth)`, path, 0, []);
    }

    if (Array.isArray(nodeValue)) {
      const total = nodeValue.length;
      ...
      if (total > 50) {
        children.push(makeNode("…", `${total - 50} more`, path ? `${path}.…` : "…", 0, []));
      }
      return makeNode(label, `array (${total})`, path, total, children);
    }
    ...
    if (nodeValue && typeof nodeValue === "object") {
      ...
      return makeNode(label, `object (${total})`, path, total, children);
    }
    ...
    return makeNode(label, `${t}${preview ? `: ${preview}` : ""}`, path, 0, []);
  };
...
function pruneTreeForExpansion(root: GraphNode, expandedPaths: Set<string>, defaultExpandDepth: number): GraphNode {
  const visit = (node: GraphNode, depth: number): GraphNode => {
    const expanded = depth < defaultExpandDepth || expandedPaths.has(node.path);
    if (!expanded || node.children.length === 0) {
      return { ...node, children: [] };
    }
    return { ...node, children: node.children.map((c) => visit(c, depth + 1)) };
  };
  return visit(root, 0);
}
...
export function DataGraph({
  ...
  const { root: fullRoot, truncated } = useMemo(
    () => buildTree(value, rootLabel, maxDepth, maxNodes),
    [value, rootLabel, maxDepth, maxNodes]
  );
  const safeExpandedPaths = useMemo(() => expandedPaths ?? new Set<string>(), [expandedPaths]);
  const visibleRoot = useMemo(
    () => pruneTreeForExpansion(fullRoot, safeExpandedPaths, defaultExpandDepth),
    [fullRoot, safeExpandedPaths, defaultExpandDepth]
  );
  const { positioned, edges, width, height, constants } = useMemo(() => layoutTree(visibleRoot), [visibleRoot]);
    ...
    <div className={cn("rounded-xl border border-gray-200 bg-white overflow-auto", className)}>
      ...
      <svg width={width} height={height} role="img" aria-label="Data graph">
        ...
        {positioned.map((n) => {
          ...
          const isExpandable = n.childCount > 0 && typeof onTogglePath === "function";
          const isExpanded = n.depth < defaultExpandDepth || safeExpandedPaths.has(n.path);
          return (
            <g
              key={n.id}
            ...
            >
              ...
              {isExpandable && (
                <g
                  transform={`translate(${constants.NODE_W - 28}, 12)`}
                  onClick={(e) => {
                    e.stopPropagation();
                    onTogglePath?.(n.path);
                  }}
                ...
                >
                  ...
                  <circle cx={10} cy={14} r={10} fill="rgba(255,255,255,0.85)" stroke="rgba(37, 99, 235, 0.45)" strokeWidth={2} />
                  <text x={10} y={18} fontSize={12} textAnchor="middle" fill="rgba(37, 99, 235, 0.9)" fontWeight={900}>
                    {isExpanded ? "−" : "+"}
                  </text>
                </g>
              )}
